...
% anonymize; switch true to false to disable
\usepackage[false]{anonymous-acm}

\title{Princeton Geniza Project datasets\footnotemark}
\authoranon{\author{Rebecca Sutton Koeser and Marina Rustow\footnotemark}}
...
\maketitle
\footnotetext{Supporting data and code available at \textanon{https://github.com/rlskoeser/pgp-dataset-essay}{https://anonymous.4open.science/r/pgp-dataset-essay-9036}}
\footnotetext{\textanon{The authors would like to thank Mohamed Abdellatif, Amel Bensalim, Rachel Richman, Ksenia Ryzhova, Laure Thompson and Jeri Wieringa for their helpful comments and suggestions on previous drafts of this essay, and for Ksenia Ryzhova for her logistical support during the final stages of completion.}{<Acknowledgements omitted for peer review>}}
...
The process of rebuilding the PGP began in July 2020 and is now nearly complete—a five-year process from start to finish. But the rewards came long before the work wrapped up. In May 2021, \textanon{we}{the team} launched the new admin interface, and the pace at which \textanon{the team}{project researchers} added documents to the database rapidly accelerated. A good research infrastructure should encourage and inspire research, and \textanon{ours}{this} quickly did just that, facilitating the production of robust datasets. 
...
To simplify the data structure, \textanon{we}{the team} modeled languages and scripts as a single entity. For instance, Judaeo-Arabic is a single entity, not Arabic language and Hebrew script. Table \ref{table:langs_docs} shows the languages that appear on the largest number of documents. (Documents often use more than one language, so these totals may include the same documents more than once.) 
...
\footnotetext{For more on UpSet plots and the challenges of visualizing overlapping combinations when there are more than four sets, read \textanon{Koeser's }{ }\citetitle{koeser_visualizing_2020}.}
...
For instance, JTS holds 10.75\% of the geniza, but it is a small institution without the technical infrastructure and staffing to host IIIF images. In this case, by building on the Princeton Geniza Lab’s previous agreements to share JTS materials for research, \textanon{we}{the team} developed an agreement enabling the DPUL to host and publish JTS images via IIIF. In another case, the Bodleian Library at the University of Oxford, with more than 12,000 fragments, publishes digitized images and metadata available online with permissive licenses, which enabled \textanon{us}{the team} to convert their XML metadata into IIIF manifests and lean on PUL infrastructure to serve the images as IIIF. In yet another case, the University of Manchester images are available as IIIF, but each image is published separately with references to their “obverse” image. Since IIIF allows for remixing, the PGPv4 team wrote scripts to combine these images into the more common structure of one manifest per fragment. These manifests preserve information about original ownership and rights; images that were already published as IIIF use a a \texttt{partOf} relationship to link back to the original manifest.
