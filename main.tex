...
\def\totalJoins{1,544 }
\def\totalLetter{11,198 }
\def\totalLegalDocument{8,173 }
\def\totalListOrTable{5,400 }
\def\totalUnknown{4,180 }
\def\totalLiteraryText{2,341 }
\def\totalStateDocument{2,080 }
\def\totalParaliteraryText{1,125 }
\def\totalCreditInstrumentOrPrivateReceipt{551 }
\def\totalLegalQueryOrResponsum{30 }
...
\def\documentsNoLang{6,200 }
\def\documentsOneLang{22,319 }
\def\documentsManyLangs{6,559 }
\def\documentsMultiLang{6,559 }
\def\documentsAnyLang{28,878 }
...
%% TODO: chart showing accumulation of pgp documents over time
...
\begin{figure}[h!]
  \includegraphics[width=\textwidth]{db-diagrams/pgp v4.19 documents.png}
  \centering
...
\begin{center}
  \includegraphics[width=0.6\textwidth]{charts/documents_per_num_frags.pdf}
  \centering
  \caption{Documents by number of fragments}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{charts/fragments_with_num_docs.pdf}
  \centering
...
Each document entry in the database includes three main metadata components: types, descriptions, and tags.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\linewidth]{charts/metadata_known_unknown.pdf}
    \caption{Metadata fields and availability across documents}
    \label{fig:metadata-status}
\end{figure}

\textbf{Types} are ways of categorizing documents according to their textual structure. Each document is categorized under a single type. There are nine types the PGP uses an intentionally simplified set of categories that Krakowski and Rustow developed over the course of several years of discussion and workshopping with other colleagues. The simplicity of this typology makes it easier to enter new documents into the database and to group like with like. 
...
\begin{figure}[h]
  \centering
  \includegraphics[width=1.0\textwidth]{charts/document_types.pdf}
...
\begin{figure}[hbt]
  \centering
  \includegraphics[width=\textwidth]{charts/primary_language_upsetplot.png}
  \caption{An UpSet plot of primary language combinations, for languages that occur at least 200 times.}
    \medskip
    \small
    The upper bar chart shows the number of documents with different set of languages or language combinations; the bar chart at left shows the size of each individual category. The provide a visual indicator of the categories at left and the combinations represented in the upper bar chart.\footnotemark
...
Situating documents chronologically is essential for historians, because it allows them to interpret them more accurately and contextualize them. But fragmentary documents can be challenging to date; Rustow has written elsewhere about the challenges and different types of uncertainty in dating geniza documents \autocite{rustow_dating_nodate}. Even documents containing explicit dates sometimes note the day and month but omit the year, which in the case of letters was usually obvious to both sender and recipient. 

When a document explicitly notes the date, we call it “date on document” and consider it authoritative. This isn’t always the date on which the document was written, but it is usually close enough to serve as a reference point. Documents rarely have more than one date; when they do, we use whichever one is latest. In other cases, the date can be inferred based on the people or events the document mentions, or coins it mentions that were minted and circulated in limited periods \autocite{dudley_coins_2023}. Some scholars are able to infer the date of documents based on handwriting, either because an individual’s writing is well-known or because the style of writing occurred only in a certain period. For instance, the scribe Ḥalfon b. Menashshe, a Jewish legal clerk in Fustat active in the first half of the twelfth century, has a distinctive handwriting that allows for confident document dating \autocite{elbaum_halfon_nodate}; a highly cursive Iberian style of writing became common in Egypt starting in the late fifteenth century.

The PGP currently has 4,305 dated documents, of which 4,005 have a date on the document itself and 300 documents have inferred dates. 

Most of the surviving dates on documents use non-Gregorian calendars. The year systems include the Jewish year (\textit{anno mundi}), the Seleucid year, the Islamic (\textit{hijrī}) year and the Egyptian fiscal (\textit{kharājī}) year. Months include the Jewish, Coptic and Islamic months, and sometimes a single date will mix and match calendars. 

The PGP supports multiple calendars; starting with version 4.5 (released in June 2022), PGP provides automatic parsing and conversion for Seleucid, \textit{hijrī} and \textit{anno mundi} dates to Common Era dates (Julian before 1583, Gregorian after). This enables documents to be sorted, filtered, and compared in a unified chronology, and to be understood by non-expert audiences.\footnote{Calendar conversion is implemented using the Python \texttt{convertdate} library, with additional logic to handle the Seleucid calendar.}

In both the PGPv4 web interface and the published datasets, we preserve and publish both the original calendar and the converted dates, since they are useful for different kinds of analysis.

\subsection{Scholarship Records}

One of the driving principles of PGPv4 was that any encounter of a human mind with a geniza document is worth preserving: the material is so challenging that it helps seeing prior attempts to understand it, even when they were inconclusive. Goitein understood this principle deeply, and that seems to have been one of his motivations for retaining all his research notes in such an organized and systematic fashion.\footnote{See Goitein's \citetitle{goitein_involvement_1974}, especially section 4, which in retrospect reads uncannily like a blueprint for PGPv4.} Like other digital humanities projects, PGP is both “a work of scholarship and an instrument of scholarship,” publishing and building on existing research while also enabling new scholarship \autocite[2]{kotin_world_2024}.
...
PGPv4 attaches evidence or sources to database entries using a database footnotes module, which we adapted from previous projects developed at CDH \autocite{koeser_princeton-cdhmep-django_2022, koeser_derrida-django_2021}.\footnote{CDH footnotes modules were originally inspired by Jean Bauer’s approach to database footnotes, which were designed to allow for conflicting evidence, indicating “whether or not that source supports the information in the record” (\citeyear{noauthor_tales_2012}).} They are implemented with Django’s “Generic Foreign Key”, which uses a combination of content type and object identifier to identify any record in the database, for example the document database table, and a document record primary key or PGPID. In the Shakespeare and Company Project, this approach was used to provide references to the archival source materials the data draws on \autocite[18]{kotin_shakespeare_2022}. In this project, we augmented the footnotes by adding a field indicating how the source relates to a specific document and whether it provides a transcription, translation, and/or discussion of it. References are included in the footnote data export as plain text.
...
By default, the site displays images, transcriptions and translations side-by-side (provided that all three exist for a given document). Users may choose to suppress any one of them from the view temporarily. The dataset exports contain transcriptions and translations as …

Users will notice symbols in the transcriptions and translations such as square brackets, parentheses, curly brackets, ellipses and double brackets. Over the past century, a set of best practices has evolved for scholarly transcription known as the Leiden conventions that use, for instance, square brackets to enclose the editor’s suggested reconstruction of missing text (as when the manuscript is torn, faded or abraded).\footnote{https://en.wikipedia.org/wiki/Leiden\_Conventions} In 2021, the PGP team developed a more streamlined version of the Leiden conventions that it uses for new transcriptions. (Some of the older, printed transcriptions in the PGP use a slightly different version of the Leiden conventions; the team has not yet brought them into line with its updated version.)

\subsection{Images of Fragments}

It is much easier to make sense of geniza documents when you can access the fragments themselves or digital images of them. The visual information contained in a fragment can enable an experienced geniza scholar to determine the genre of the text at a glance, and whether it was an official version of a document or the notes of a scribe preparing to write one. High resolution images also enable scholars to correct the transcriptions that previous generations had made from photstats, microfilms or even the originals. For all those reasons, PGPv4 was determined to include digital images, and to capitalize on the affordances of the International Image Interoperability Framework (IIIF) APIs.

IIIF provides a standardized mechanism for linking to images managed and preserved by multiple institutions and is seeing increasing adoption by libraries and museums around the world. The three largest \textit{geniza }collections in the world have made their materials available in IIIF-compliant images: tens of thousands of fragments from the Cambridge University Library, which houses around 200,000 fragments; all of the 43,000 fragments at the Jewish Theological Seminary (JTS) in New York and all of the 17,000 fragments at the University of Manchester. (The remaining collections will be made publicly available online and IIIF-compliant over the coming years by the National Library of Israel.) For all these reasons, we chose to leverage IIIF APIs for referencing and displaying images whenever possible.

We decided to support IIIF images only because we did not have the capacity to publish and preserve images as part of the PGPv4 application—or want to, given that qualified institutions are already handling that important work. In some cases, our commitment to IIIF required us to wait for institutions to migrate digitized images into more modern systems (for example, the\textit{ geniza} materials at the University of Pennsylvania, which we were able to import into PGPv4 in 2023). In other cases, IIIF image publication required creativity and additional organizational and technical work.

For instance, JTS holds 10.75\% of the geniza, but because it is a smaller institution without the technical infrastructure to host IIIF images, we built on the Princeton Geniza Lab’s previous agreements to share their materials for research and brokered an agreement enabling the Princeton University Library (PUL) to host and publish their images, making them IIIF compliant. In another case, the Bodleian Library at the University of Oxford (more than 12,000 fragments) made its digitized images and metadata available online with permissive licenses, but they were not IIIF compliant, so we wrote scripts to convert their XML into IIIF manifests, leaning on the PUL infrastructure to serve the images as IIIF. In yet another case, the University of Manchester images were already IIIF compliant, but using a different structure from the one that the PGPv4 had assumed based on Cambridge’s images: rather than one manifest per fragment, with front and back images grouped as in Cambridge, Manchester had published each image in its own manifest, with identifiers linking to the “obverse” image. Rather than complicate the PGPv4 code to support this new image data structure, we wrote custom scripts to remix these image manifests to match the structure we had assumed. Since IIIF manifests are simple JSON documents, these are published statically on GitHub pages and reference the IIIF images served out by Manchester. In all cases, we have preserved and displayed the image ownership records and the reuse permissions clearly, both in our revised manifests and alongside the images as they are displayed on the PGPv4 site.

One of the benefits of IIIF is that it makes it possible to display multiple fragments for a single document, even if they are from different collections—indispensable for displaying joins. Moreover, the PGPv4 admin interface allows data curators to suppress images in order to display only those relevant for a particular document—for example, omitting the verso if only the recto is needed—and to reorder and rotate them so that they display in the correct sequence and orientation. The PGPv4 system does not currently support stitching fragment images together, although in principle it could with further software development; but it is still invaluable to see all the fragments of a single document together, even when they are held by different libraries.

[note: data exports don’t currently include our arrangement of the images with these overrides; I propose we describe that as being included in a future version of of the datasets so it isn’t a blocker to submitting this essay for review; hopefully they’ll be included by the time we’re revising for publication]

\subsection{People and Places}

In November 2024, the PGP team released a major update, v4.18, which added two new modules to the site: People and Places. These modules allow the team to create records for any location or individual appearing in the documentary \textit{geniza }corpus. People and places are accessible from document records or from their own sections of the PGP website.

The People module now includes more than 1,100 records for people from PGP documents. Each record includes links to the documents in which that person appears, as well as their social role(s) and their relationship to the document (recipient of a letter, for example, or signatory on a legal deed). In the dataset, …

The People module has two main purposes. The first is to enable researchers to build network graphs to identify related people, whether extended families, trade networks or members of the congregation that prayed in the medieval synagogue where the Cairo Geniza survived. Its second purpose is to help researchers determine the date-ranges of undated documents. If an undated document mentions someone who also appears in documents dated between 1111 and 1137, then we have a clue to that document’s inferred date-range.

As for the Places module, it now records nearly 300 historical place names related to PGP documents, with GIS coordinates to allow for geographical analysis. An in-progress update to the project will distinguish between specific sites such as cities, towns and villages, and less specific regions. Each place is linked to the documents in which it appears, as well as some of the people connected with it; Fustat, the medieval residential core of Cairo, unsurprisingly has the largest number of linked documents so far, 331, although this is very likely a gross undercount; a more indicative number is Raʾs Tīnī in Libya, with seven documents; xxx places have just one document listed each. Once the team has more fully populated the data in the Places module, they will be able to determine which places other than Fustat and Cairo are most commonly mentioned in \textit{geniza }documents—providing, in turn, a valuable quantitative check on the field’s impressionistic representation of certain places as important, such as Alexandria, Jerusalem, Damascus and Palermo.
...
\begin{itemize}
    \item Description of plan and rationale for regular publication of datasets (probably quarterly; for official citable versions, replication, check points for comparison over time)
    \item provide some summary analysis/viz of how the datasets have changed over time so far, since we’ve been collecting them on github, to give some sense of the volatility and growth
    \begin{itemize}
        \item E.g., total number of documents, how many added/modified by week
        \item From 2024-03-06 meeting: \# of documents, \# transcriptions, \# translations (\# images?) over time. (tags? Descriptions / description length?)
    \end{itemize}
\end{itemize}
